...
async function replace (opCtx: any, doc: any, storageDoc: any, options: any) {
  ...
  if (storageDoc.identifier !== doc.identifier || isDeduplication) {
    ...
    // @ts-expect-error TS(2339): Property 'identifier' does not exist on type '{ la... Remove this comment to see the full error message
    fields.identifier = doc.identifier;
    // @ts-expect-error TS(2339): Property 'isDeduplication' does not exist on type ... Remove this comment to see the full error message
    fields.isDeduplication = true;
    if (storageDoc.identifier !== doc.identifier) {
      // @ts-expect-error TS(2339): Property 'deduplicatedIdentifier' does not exist o... Remove this comment to see the full error message
